...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:29Malysana@localhost/camchanHW3"
## Provided:
...
def index():
				...
				## And redirect to the list of all tweets
								t = Tweet.query.filter_by(text=text,user_id=user.id).first()
								if t:
...
def see_all_tweets():
	...
	for tweet in tweets:
		...
		tweet_id = User.query.filter_by(id=tweet.user_id).first().username
		tweet_tup = (tweet_text,tweet_id)
		all_tweets.append(tweet_tup)
...
def get_longest_tweet(): 
    ...
    for tweet in tweets:
    	...
    	for ch in tweet.text:
    		if ch != ' ':
    			ch_count += 1
    	tweet_dict[tweet.text] = ch_count
    sorted_tweets = sorted(tweet_dict.items(),key=lambda x: x[1],reverse=True)
    sorted_tweet = sorted_tweets[0][0]
    longest_tweet = Tweet.query.filter_by(text=sorted_tweet).first()
    username = User.query.filter_by(id=longest_tweet.user_id).first().username
    display_name = User.query.filter_by(id=longest_tweet.user_id).first().display_name
    return render_template('longest_tweet.html',text=sorted_tweet,user=username,display_name=display_name)
# NOTE:
